...
class JobManager:
    ...
    def submit(self, tool_name: str, fn: Callable[[], Any]) -> dict[str, Any]:
        self._cleanup()
        rec = JobRecord(job_id=f"job_{uuid.uuid4().hex[:16]}", tool_name=tool_name)
        with self._lock:
            self._jobs[rec.job_id] = rec
            self._persist(rec)
        ...
        with self._lock:
            ...
            if rec.status == "running":
                ...
                body.update({
                    ...
                    "message": "running Brightway jobs cannot be interrupted safely",
                })
                return body
    ...
    def list_jobs(self, limit: int = 20) -> dict[str, Any]:
        self._cleanup()
        with self._lock:
            rows = sorted(self._jobs.values(), key=lambda rec: rec.created_at, reverse=True)
        return {"success": True, "jobs": [rec.public() for rec in rows[:min(100, max(1, limit))]]}
...
jobs = JobManager(
    max_workers=int(os.getenv("BRIGHTWAY_JOB_WORKERS", "1")),
    ttl_seconds=int(os.getenv("BRIGHTWAY_JOB_TTL_SECONDS", "3600")),
)
